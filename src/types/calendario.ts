import { CALENDARIO_ORIGEN, CALENDARIO_ORIGEN_CUMPLEANIOS_MISIONERO, type CalendarioOrigen } from '@/lib/constants/calendario';

export type CalendarioEstado = 'activo' | 'cancelado';

export type CalendarioOrigenEvento = CalendarioOrigen | typeof CALENDARIO_ORIGEN_CUMPLEANIOS_MISIONERO;
...
export type ActividadCalendario = {
  ...
  tipo: string;
  origen_tipo: CalendarioOrigenEvento;
...
export const CALENDARIO_COLORS: Record<CalendarioOrigenEvento, { bg: string; border: string; text: string }> = {
  ...
  [CALENDARIO_ORIGEN_CUMPLEANIOS_MISIONERO]: { bg: '#E7F7EF', border: '#2F855A', text: '#1F5C3D' },
};
