...
import { useToggleAsistenciaMisionero, useEliminarAsistenciaMisionero } from '@/lib/queries/formaciones';
...
export function AsistenciasTab({ formacionId }: { formacionId: string }) {
  const { data, isLoading } = useAsistenciasFormacion(formacionId);
  ...
  if (clases.length === 0 || inscriptos.length === 0) {
    return <p className="text-brand-brown text-sm">No hay clases o misioneros inscriptos aún</p>;
  }
